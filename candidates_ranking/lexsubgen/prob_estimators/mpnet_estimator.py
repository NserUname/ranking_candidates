...
class MpnetProbEstimator(EmbSimProbEstimator):
    def __init__(
        self,
        ...
        """
        super(MpnetProbEstimator, self).__init__(
    ...
    def register_model(self) -> NoReturn:
        ...
        """
        if self.model_name not in MpnetProbEstimator.loaded:
            self.model_name = '/home/ustc_zyhu/LexSub/transformer_models/'+self.model_name
            roberta_model = AutoModelForMaskedLM.from_pretrained(self.model_name)
            roberta_model.to(self.device).eval()
            roberta_tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            roberta_word2id = MpnetProbEstimator.load_word2id(roberta_tokenizer)
            filter_word_ids = MpnetProbEstimator.load_filter_word_ids(
            ...
            MpnetProbEstimator.loaded[self.model_name] = {
                "model": roberta_model,
            ...
            MpnetProbEstimator.loaded[self.model_name]["ref_count"] = 1
        else:
            MpnetProbEstimator.loaded[self.model_name]["ref_count"] += 1

...
class SimCSEProbEstimator(EmbSimProbEstimator):
    # 类属性，用于存储加载的词嵌入和分词器
    ...
    def __init__(
        ...
        generation_otherPosition_ways:str="maskProb",    # 'maskProb', 'ig', 'attention','target'         其余模型要添加此处
        generation_otherPosition_ways_mode:str="mask" 
        ...
        """
        super(SimCSEProbEstimator, self).__init__(
        ...
        # 检查类属性是否已经加载了词嵌入和分词器
        if SimCSEProbEstimator._word_embeddings is None or SimCSEProbEstimator._tokenizer is None:
            SimCSEProbEstimator._word_embeddings = torch.tensor(
                self.loaded[self.model_name]["embeddings"], device=self.device
            )
            SimCSEProbEstimator._tokenizer = self.loaded[self.model_name]["tokenizer"]
    ...
    def register_model(self) -> NoReturn:
        ...
        """
        if self.model_name not in SimCSEProbEstimator.loaded: # 父类的loaded
            self.model_name = '/home/ustc_zyhu/LexSub/transformer_models/'+self.model_name

            bert_model = AutoModel.from_pretrained(self.model_name)       # 实现了模型加载，字典
            bert_model=bert_model.to(self.device).eval()                        # 指定设备
            bert_tokenizer = AutoTokenizer.from_pretrained(
                self.model_name, do_lower_case=self.model_name.endswith("uncased")
            )
            bert_word2id = SimCSEProbEstimator.load_word2id(bert_tokenizer)
            bert_filter_word_ids = SimCSEProbEstimator.load_filter_word_ids(
                bert_word2id, punctuation
            )
            word_embeddings = (
                bert_model.embeddings.word_embeddings.weight.detach()
            )
            # 这里将模型的loaded中传入了，因此父类中就可以直接调用了
            SimCSEProbEstimator.loaded[self.model_name] = {
                "model": bert_model,
            ...
            SimCSEProbEstimator.loaded[self.model_name]["ref_count"] = 1
        else:
            SimCSEProbEstimator.loaded[self.model_name]["ref_count"] += 1
    ...
    def get_unk_word_vector(self, word) -> np.ndarray:
        ...
        if self.use_subword_mean:
            ...
            return mean_vector
        return super(SimCSEProbEstimator, self).get_unk_word_vector(word)
    

       
    @staticmethod
    def load_word2id(tokenizer: AutoTokenizer) -> Dict[str, int]:
        """
    ...
    def bert_tokenize_sentence(
        self, tokens: List[str], tokenizer: AutoTokenizer = None
    ) -> List[str]:
        """
    ...
    def predictByTarget(self,  tokens_lists: List[List[str]], target_ids: List[int],target_pos_tag: List[str]):
        ...
        logits_tensor = torch.full((len(tokens_lists),len(self.word2id)), 0.1)
    ...
    def getOtherPositionConcatScoreByIg(self, tokens_lists = None, pred_substitutes = None, target_ids = None):
        ...
        for start in range(0, input_ids.size(0), ig_batch):
            ...
            def forward_func(input_embeds):
                # 直接传 embeddings，返回所有层 hidden states
                outputs = self.model(inputs_embeds=input_embeds, output_hidden_states=True, return_dict=True)
                hidden_states = outputs.hidden_states[start_layer:end_layer]   # tuple: (layer0, layer1, ..., layerN)
                # 每一层维度: (batch_size, seq_len, hidden_size)

                batch_scores = []
                for b, (row, col) in enumerate(mask_indices_batch):
                    # 同一个位置，取所有层的 hidden state 并拼接
                    concat_repr = torch.cat([hs[b, col, :] for hs in hidden_states], dim=-1)
                    # 压缩为标量（这里用 L2 范数）
                    score = torch.norm(concat_repr, p=2)
                    batch_scores.append(score)

                return torch.stack(batch_scores)  # (batch_size, hidden_size * num_layers)

            lig = LayerIntegratedGradients(forward_func, self.model.embeddings)

            baseline_ids = torch.full_like(input_ids_batch, self.tokenizer.pad_token_id).to(self.device)
            baseline_embeds = self.model.embeddings(baseline_ids)
            input_embeds_batch = self.model.embeddings(input_ids_batch)
    ...
    def predictByOther(self,  tokens_lists: List[List[str]], target_ids: List[int],target_pos_tag: List[str]):
        ...
        # 获取原始嵌入
        with torch.no_grad():
            embeddings = self.model.embeddings(input_ids)  # 原始嵌入 [batch_size, seq_len, hidden]
            # 遍历处理每个样本
            ...
            for idx in range(batch_size):
                ...
                for pos in positions[idx]:
                    if self.generation_otherPosition_ways_mode == "mask":
                        ...
                        new_embeddings = self.model.embeddings(new_input_ids)
    ...
    def getTargetPositionConcatScoreV2(self, tokens_lists=None, pred_substitutes=None, target_ids=None):
        """
        Args:

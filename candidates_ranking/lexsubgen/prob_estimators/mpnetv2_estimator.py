...
class MpnetV2ProbEstimator(EmbSimProbEstimator):
    def __init__(
        self,
        ...
        """
        super(MpnetV2ProbEstimator, self).__init__(
    ...
    def register_model(self) -> NoReturn:
        ...
        """
        if self.model_name not in MpnetV2ProbEstimator.loaded:
            self.model_name = '/home/ustc_zyhu/LexSub/transformer_models/'+self.model_name
            roberta_model = AutoModelForMaskedLM.from_pretrained(self.model_name)   # 句子向量模型，可能不输出logits，所以SimCSE
            roberta_model.to(self.device).eval()
            roberta_tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            roberta_word2id = MpnetV2ProbEstimator.load_word2id(roberta_tokenizer)
            filter_word_ids = MpnetV2ProbEstimator.load_filter_word_ids(
                roberta_word2id, punctuation
            )
            word_embeddings = (
                roberta_model.mpnet.embeddings.word_embeddings.weight.detach()
            )

            # norms = np.linalg.norm(word_embeddings, axis=-1, keepdims=True)
            # normed_word_embeddings = word_embeddings / norms

            MpnetV2ProbEstimator.loaded[self.model_name] = {
                "model": roberta_model,
            ...
            MpnetV2ProbEstimator.loaded[self.model_name]["ref_count"] = 1
        else:
            MpnetV2ProbEstimator.loaded[self.model_name]["ref_count"] += 1
    ...
    def getOtherPositionConcatScoreByIg(self, tokens_lists = None, pred_substitutes = None, target_ids = None):
        ...
        for start in range(0, input_ids.size(0), ig_batch):
            ...
            lig = LayerIntegratedGradients(forward_func, self.model.mpnet.embeddings)
            
            baseline_ids = torch.full_like(input_ids_batch, self.tokenizer.pad_token_id).to(self.device)
            baseline_embeds = self.model.mpnet.embeddings(baseline_ids)
            input_embeds_batch = self.model.mpnet.embeddings(input_ids_batch)
